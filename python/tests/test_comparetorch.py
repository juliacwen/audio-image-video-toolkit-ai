...
def test_comparetorch_csv_runs_identical():
    start, limit = safe_start_and_limit(samples)
    result = run_compare("python/audio/comparetorch_csv.py", samples, samples, start, limit)
    assert result.returncode == 0, result.stderr
    assert "Traceback" not in result.stderr

def test_comparetorch_csv_runs_different():
    start, limit = safe_start_and_limit(samples)
    result = run_compare("python/audio/comparetorch_csv.py", samples, samples5, start, limit)
    assert result.returncode == 0, result.stderr
    assert "Traceback" not in result.stderr
