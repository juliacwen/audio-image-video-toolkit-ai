#!/usr/bin/env python3
# Multimodal/bayesian/app_bayesian_fft_streamlit_db.py
...
Stores results directly in `multimodal.db` at project root.
...
Run the app from the project root:
    ...
    streamlit run Multimodal/bayesian/app_bayesian_fft_streamlit_db.py
...
TEST_FILES_DIR = os.path.join(BASE_DIR, "../llm/test/test_files")
...
if not audio_files:
    ...
    ax_time.plot(t1_zoomed, sig1_zoomed, color=LINE_COLOR1, label=f"{selected_file1} waveform")
    if sig2 is not None:
        ax_time.plot(t2_zoomed, sig2_zoomed, color=LINE_COLOR2, label=f"{selected_file2} waveform")
    ...
    ax_fft.plot(freqs1, fft_vals1, label=f"{selected_file1} FFT", color=LINE_COLOR1)
    ax_fft.axvline(mean_peak1, color=DASHED_COLOR1, linestyle='--', label=f"{selected_file1} mean peak")
    if compare_mode and freqs2 is not None:
        ax_fft.plot(freqs2, fft_vals2, label=f"{selected_file2} FFT", color=LINE_COLOR2)
        ax_fft.axvline(mean_peak2, color=DASHED_COLOR2, linestyle='--', label=f"{selected_file2} mean peak")
    ...
    ax_hist.hist(samples1, bins=HIST_BINS, color=LINE_COLOR1, alpha=0.5, edgecolor='black',
                 label=f"{selected_file1} samples")
    ax_hist.axvline(mean_peak1, color=DASHED_COLOR1, linestyle='--', label=f"{selected_file1} mean peak")
    if compare_mode and samples2 is not None:
        ax_hist.hist(samples2, bins=HIST_BINS, color=LINE_COLOR2, alpha=0.5, edgecolor='black',
                     label=f"{selected_file2} samples")
        ax_hist.axvline(mean_peak2, color=DASHED_COLOR2, linestyle='--', label=f"{selected_file2} mean peak")
